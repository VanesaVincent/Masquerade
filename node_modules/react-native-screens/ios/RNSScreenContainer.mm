...
- (void)markChildUpdated
{
  ...
  // by the changes of `Animated` value in stack's transition or adding/removing assets_screens
  // in all navigators
  RCTAssertMainQueue();
...
- (void)updateContainer
{
  ...
  // remove assets_screens that are no longer active
  ...
  if (screenAdded) {
    // add new assets_screens in order they are placed in subviews array
    NSInteger index = 0;
    for (RNSScreenView *screen in _reactSubviews) {
      if (screen.activityState != RNSActivityStateInactive) {
        if ([_activeScreens containsObject:screen] && screen.activityState == RNSActivityStateTransitioningOrBelowTop) {
          // for assets_screens that were already active we want to mimick the effect UINavigationController
