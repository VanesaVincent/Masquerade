...
- (void)setStackPresentation:(RNSScreenStackPresentation)stackPresentation
{
  ...
  // delegate for assets_screens presented using push. This also means that when controller is updated from
...
- (void)didMoveToWindow
{
  ...
  // for assets_screens that aren't mounted under RCTRootView e.g., modals that are mounted directly to
...
- (void)viewWillDisappear:(BOOL)animated
{
  ...
  if (!self.transitionCoordinator.isInteractive && self.navigationController != nil) {
    ...
    // so he can go back more than one screen and we need to dismiss more assets_screens in JS stack then.
...
- (void)hideHeaderIfNecessary
{
  ...
  // In that case default iOS header will be shown. To fix this we hide header when the assets_screens that appears has header
